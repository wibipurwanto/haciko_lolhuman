...
const { menu } = require("../utils/message");
...
module.exports = async (sock, msg) => {
  ...
  if (!user) {
    users[from] = { limit, name: "Anonim" }; // Default nama jika tidak ada
    user = users[from];
  ...
  if (!user.limit && isCmd) {
    return reply(
      "Limit sudah terpakai habis.\nHubungi owner untuk menambah limit, Dan bisa menggunakan fitur"
    );
  }
  ...
  if (
    ...
    return reply(
      `Command cooldown, Anda dapat menggunakannya lagi setelah ${Math.round(
        duration.asSeconds()
      )}s`
    );
  } else if (
    !cooldown.get(cooldownBuilder) ||
    (cooldown.get(cooldownBuilder) && cooldown.get(cooldownBuilder) < moment())
  ) {
    cooldown.set(
      cooldownBuilder,
      moment().add(moment.duration(3000, "milliseconds"))
    ); // 3 seconds
  ...
  switch (command) {
    ...
    case "help":
      ...
      break;
    case "ceklimit":
      reply(`Limit pengguna: ${user.limit}`);
      break;
    case "cektotaluser":
      let userList = "Daftar pengguna dan limit:\n";
      for (const userNumber in users) {
        userList += `${users[userNumber].name}: ${users[userNumber].limit}\n`;
      }
      reply(userList);
      break;
    case "cast":
      if (!isOwner) {
        return reply("Hanya owner yang dapat menggunakan perintah broadcast.");
      }

      const broadcastMessage =
        "Ini adalah pesan broadcast kepada semua pengguna.";
      sendBroadcast(broadcastMessage);

      // Fungsi untuk mengirim pesan broadcast ke semua pengguna
      function sendBroadcast(message) {
        Object.keys(users).forEach((userNumber) => {
          const user = users[userNumber];
          const messageType = "text"; // Anda dapat mengganti dengan jenis pesan yang sesuai (contoh: "image", "video", dll)

          // Membuat objek pesan sesuai dengan jenis pesan yang Anda inginkan
          const messageObject =
            messageType === "text"
              ? { text: message }
              : {
                  /* konfigurasi sesuai jenis pesan yang Anda inginkan */
                };

          // Mengirim pesan ke pengguna
          sock
            .sendMessage(userNumber, messageObject)
            .then(() => {
              console.log(
                `Pesan broadcast terkirim ke ${user.name} (${userNumber}).`
              );
            })
            .catch((error) => {
              console.error(
                `Gagal mengirim pesan broadcast ke ${user.name} (${userNumber}): ${error}`
              );
            });
        });
      }
    ...
    case "daftar":
      if (args.length > 0) {
        ...
        users[from].name = newName;
        writeDatabase("users", users);
        reply(`Halo, ${newName}! Anda telah terdaftar.`);
      } else {
        reply("Format perintah salah. Gunakan: daftar [Nama Anda]");
      }
      break;
    case "setlimit":
      if (!isOwner) {
        return reply("Hanya owner yang bisa mengatur limit.");
      }

      if (args.length !== 2) {
        return reply(
          `Penggunaan: ${prefix + command} <nama_pengguna> <batasan_baru>`
        );
      }
      ...
      if (isNaN(newLimit) || newLimit < 0) {
        return reply("Batasan baru harus berupa angka positif.");
      }

      const targetUser = Object.values(users).find(
        (user) => user.name.toLowerCase() === targetName.toLowerCase()
      );

      if (!targetUser) {
        return reply("Pengguna tidak ditemukan dalam database.");
      }
      targetUser.limit = newLimit;
      writeDatabase("users", users);

      return reply(
        `Batasan pengguna ${targetUser.name} diatur menjadi ${newLimit}`
      );
      break;
    case "cekuser":
      if (mentioned.length === 0) {
        return reply("Tag pengguna yang ingin Anda cek.");
      }

      let userCheckList = "Informasi pengguna:\n";
      mentioned.forEach((mentionedUser) => {
        if (users[mentionedUser]) {
          userCheckList += `${
            users[mentionedUser].name || "Nama Tidak Tersedia"
          } (${mentionedUser}): ${users[mentionedUser].limit}\n`;
        } else {
          userCheckList += `Pengguna (${mentionedUser}) tidak terdaftar.\n`;
        }
      });
    ...
    case "cekapi":
      ...
      return axios
        .get(`https://api.lolhuman.xyz/api/checkapikey?apikey=${apikey}`)
        .then(({ data }) => {
    ...
    case "ytplay":
      if (args.length == 0)
        return await reply(`Example: ${prefix + command} melukis senja`);
      axios
        .get(
    ...
    case "ytmp3":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/ytaudio?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
          ...
          caption += `❖ Dislike  : *${data.result.dislike}*\n`;
          caption += `❖ Ukuran   : *${data.result.size}*`;

          // Pastikan data.result.thumbnail adalah string URL yang valid
          const thumbnailUrl = data.result.thumbnail;

          sock
            .sendMessage(from, {
              // Gunakan URL gambar miniatur yang valid di sini
    ...
    case "telesticker":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/telestick?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
          sock.sendMessage(from, {
            sticker: { url: data.result.sticker.random() },
          });
        });
      break;
    case "tiktokwm":
      ...
      sock.sendMessage(from, {
        video: {
          url: `https://api.lolhuman.xyz/api/tiktokwm?apikey=${apikey}&url=${args[0]}`,
        },
      });
      break;
    case "tiktoknowm":
    ...
    case "spotify":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/spotify?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
          var caption = `Title : ${data.result.title}\n`;
    ...
    case "spotifysearch":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/spotifysearch?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          var text = "";
          for (var x of data.result) {
    ...
    case "jooxplay":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/jooxplay?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          ...
          caption += `Lirik :\n ${data.result.lirik}\n`;
          sock
            .sendMessage(from, { image: { url: data.result.image }, caption })
            .then(() => {
              sock.sendMessage(from, {
                audio: { url: data.result.audio[0].link },
                mimetype: "audio/mp4",
                fileName: `${data.result.title}.mp3`,
              });
            });
    ...
    case "twtdl":
      ...
      axios
        ...
        .then(({ data }) => {
          sock.sendMessage(from, {
            video: { url: data.result.link[data.result.link.length - 1].link },
            mimetype: "video/mp4",
          });
        });
      break;
    ...
    case "zippyshare":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/zippyshare?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
    ...
    case "pinterest":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/pinterest?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
    ...
    case "pinterest2":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/pinterest2?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
    ...
    case "pinterestdl":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/pinterestdl?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
    ...
    case "pixiv":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/pixiv?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
    ...
    case "pixivdl":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/pixivdl/${args[0]}?apikey=${apikey}`,
        },
      });
      break;
    ...
    case "character":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/character?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          ...
          caption += `\nDescription : \n${data.result.description.replace(
            /__/g,
            "_"
          )}`;
          sock.sendMessage(from, {
            image: { url: data.result.image.large },
            caption,
          });
        });
      break;
    case "manga":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/manga?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          var caption = `Id : ${data.result.id}\n`;
          ...
          sock.sendMessage(from, {
            image: { url: data.result.coverImage.large },
            caption,
          });
        });
      break;
    case "anime":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/anime?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          var caption = `Id : ${data.result.id}\n`;
          caption += `Id MAL : ${data.result.idMal}\n`;
    ...
    case "wait":
      ...
      axios
        .post(`https://api.lolhuman.xyz/api/wait?apikey=${apikey}`, form)
        .then(({ data }) => {
    ...
    case "kusonime":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/kusonime?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
          var caption = `Title : ${data.result.title}\n`;
    ...
    case "kusonimesearch":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/kusonimesearch?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          var caption = `Title : ${data.result.title}\n`;
          caption += `Japanese : ${data.result.japanese}\n`;
    ...
    case "otakudesu":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/otakudesu?apikey=${apikey}&url=${args[0]}`
        )
        .then(({ data }) => {
    ...
    case "otakudesusearch":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/otakudesusearch?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          var text = `Title : ${data.result.title}\n`;
          text += `Japanese : ${data.result.japanese}\n`;
    ...
    case "storynime":
      axios
        .get(`https://api.lolhuman.xyz/api/${command}?apikey=${apikey}`)
        .then(({ data }) => {
          sock.sendMessage(from, {
            video: { url: data.result },
            mimetype: "video/mp4",
    ...
    case "cekresi":
      axios
        .get(
          `https://api.lolhuman.xyz/api/checkresi?apikey=${apikey}&resi=${args[0]}`
        )
        .then(({ data }) => {
    ...
    case "agedetect":
      ...
      form.append("img", stream, "tahu.jpg");
      axios
        .post(`https://api.lolhuman.xyz/api/agedetect?apikey=${apikey}`, form)
        .then(({ data }) => {
    ...
    case "brainly":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} siapakah sukarno`);
      var { data } = await axios.get(
    ...
    case "jarak":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} jakarta - yogyakarta`);
      var text1 = full_args.split("-")[0].trim();
      var text2 = full_args.split("-")[1].trim();
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/jaraktempuh?apikey=${apikey}&kota1=${text1}&kota2=${text2}`
      );
    ...
    case "urbandictionary":
      var { data } = await axios.get(
        `http://lolhuman.herokuapp.com/api/urdict?apikey=${apikey}&query=${full_args}`
      );
      for (var x of data.result) {
        var text = `\`\`\`Meaning :\n${x.definition}\`\`\`\n\n`;
    ...
    case "chord":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} Melukis senja`);
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/chord?apikey=${apikey}&query=${full_args}`
      );
      var text = `Title : ${data.result.title}\n`;
      text += `Chord : \n${data.result.chord}`;
      reply(text);
    ...
    case "heroml":
      ...
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/heroml/${full_args}?apikey=${apikey}`
      );
    ...
    case "mlstalk":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} 84830127/2169`);
      var { data } = await axios.get(
    ...
    case "qrreader":
      ...
      var { data } = await axios.post(
        `https://api.lolhuman.xyz/api/read-qr?apikey=${apikey}`,
        form
      );
    ...
    case "translate":
      ...
      var text = args.join(" ");
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/translate/auto/${kode_negara}?apikey=${apikey}&text=${text}`
      );
    ...
    case "covidindo":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/corona/indonesia?apikey=${apikey}`
      );
    ...
    case "covidglobal":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/corona/global?apikey=${apikey}`
      );
      var text = `Positif : ${data.result.positif}\n`;
      text += `Sembuh : ${data.result.sembuh}\n`;
      text += `Dirawat : ${data.result.dirawat}\n`;
    ...
    case "indbeasiswa":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/indbeasiswa?apikey=${apikey}`
      );
    ...
    case "hoax":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/turnbackhoax?apikey=${apikey}`
      );
      var text = "Info Hoax :\n";
      for (var x of data.result) {
        text += `Title : ${x.title}\n`;
    ...
    case "nsfwcheck":
      ...
      form.append("img", stream, { filename: "tahu.jpg" });
      var { data } = await axios.post(
        `https://api.lolhuman.xyz/api/nsfwcheck?apikey=${apikey}`,
        form
      );
      var is_nsfw = "No";
    ...
    case "ocr":
      ...
      form.append("img", stream, { filename: "tahu.jpg" });
      var { data } = await axios.post(
        `https://api.lolhuman.xyz/api/ocr?apikey=${apikey}`,
        form
      );
      reply(`Result : ${data.result}`);
      break;
    ...
    case "lk21":
      ...
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/lk21?apikey=${apikey}&query=${full_args}`
      );
      var caption = `Title : ${data.result.title}\n`;
    ...
    case "drakorongoing":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/drakorongoing?apikey=${apikey}`
      );
    ...
    case "wattpad":
      if (args.length == 0)
        return reply(
          `Example: ${
            prefix + command
          } https://www.wattpad.com/707367860-kumpulan-quote-tere-liye-tere-liye-quote-quote`
        );
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/wattpad?apikey=${apikey}&url=${args[0]}`
      );
      var caption = `Title : ${data.result.title}\n`;
      caption += `Rating : ${data.result.rating}\n`;
    ...
    case "wattpadsearch":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} Tere Liye`);
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/wattpadsearch?apikey=${apikey}&query=${full_args}`
      );
    ...
    case "cerpen":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/cerpen?apikey=${apikey}`
      );
      var text = `Title : ${data.result.title}\n`;
    ...
    case "ceritahoror":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/ceritahoror?apikey=${apikey}`
      );
      var caption = `Title : ${data.result.title}\n`;
      caption += `Desc : ${data.result.desc}\n`;
      caption += `Story :\n${data.result.story}\n`;
    ...
    case "wallpapersearch":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/${command}?apikey=${apikey}&query=${full_args}`,
        },
      });
      break;
    case "gimage2":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/gimage2?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          for (var x of data.result.slice(0, 5)) {
            sock.sendMessage(from, { image: { url: x } });
    ...
    case "wallpapersearch2":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/wallpaper2?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          sock.sendMessage(from, { image: { url: data.result } });
        });
    ...
    case "playstore":
      ...
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/playstore?apikey=${apikey}&query=${full_args}`
      );
    ...
    case "shopee":
      ...
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/shopee?apikey=${apikey}&query=${full_args}`
      );
    ...
    case "quotes":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/random/quotes?apikey=${apikey}`
      );
      reply(`_${data.result.quote}_\n\n*― ${data.result.by}*`);
      break;
    case "quotesanime":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/random/quotesnime?apikey=${apikey}`
      );
      reply(
      ...
      break;
    case "quotesdilan":
      quotedilan = await axios.get(
        `https://api.lolhuman.xyz/api/quotes/dilan?apikey=${apikey}`
      );
    ...
    case "bucin":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/random/${command}?apikey=${apikey}`
      );
      reply(data.result);
      break;
    case "randomnama":
      var { data } = await axios.get(
        `https://api.lolhuman.xyz/api/random/nama?apikey=${apikey}`
      );
      reply(data.result);
      break;
    ...
    case "asupan":
      axios
        .get(`https://api.lolhuman.xyz/api/asupan?apikey=${apikey}`)
        .then(({ data }) => {
          sock.sendMessage(from, {
            video: { url: data.result },
            mimetype: "video/mp4",
    ...
    case "artinama":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/artinama?apikey=${apikey}&nama=${full_args}`
        )
        .then(({ data }) => {
          reply(data.result);
        });
    ...
    case "jodoh":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} Tahu & Bacem`);
      axios
        .get(
          `https://api.lolhuman.xyz/api/jodoh/${full_args.split("&")[0]}/${
            full_args.split("&")[1]
          }?apikey=${apikey}`
        )
        .then(({ data }) => {
    ...
    case "weton":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} 12 12 2020`);
      axios
        .get(
          `https://api.lolhuman.xyz/api/weton/${args[0]}/${args[1]}/${args[2]}?apikey=${apikey}`
        )
        .then(({ data }) => {
    ...
    case "jadian":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/jadian/${args[0]}/${args[1]}/${args[2]}?apikey=${apikey}`
        )
        .then(({ data }) => {
    ...
    case "tebakumur":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/tebakumur?apikey=${apikey}&name=${full_args}`
        )
        .then(({ data }) => {
    ...
    case "imagetoanime":
      ...
      axios
        .post(
          `https://api.lolhuman.xyz/api/imagetoanime?apikey=${apikey}`,
          form,
    ...
    case "caristiker":
      ...
      axios
        .get(
          `https://api.lolhuman.xyz/api/stickerwa?apikey=${apikey}&query=${full_args}`
        )
        .then(({ data }) => {
          ...
          sock.sendMessage(from, {
            sticker: {
              url: `https://api.lolhuman.xyz/api/convert/towebp?apikey=${apikey}&img=${sticker}`,
            },
            caption,
          });
        });
    ...
    case "stalkig":
      ...
      axios
        .get(`https://api.lolhuman.xyz/api/stalkig/${args[0]}?apikey=${apikey}`)
        .then(({ data }) => {
    ...
    case "stalkgithub":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} LoL-Human`);
      axios
        .get(`https://api.lolhuman.xyz/api/github/${args[0]}?apikey=${apikey}`)
        .then(({ data }) => {
    ...
    case "stalktwitter":
      ...
      axios
        .get(`https://api.lolhuman.xyz/api/twitter/${args[0]}?apikey=${apikey}`)
        .then(({ data }) => {
    ...
    case "ai":
      if (args.length == 0)
        return reply(`Example: ${prefix + command} ai adalah`);
      axios
        .get(
    ...
    case "quotesimage":
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/random/${command}?apikey=${apikey}`,
        },
      });
      break;
    ...
    case "hentai4everyone":
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=${apikey}`,
        },
      });
      break;
    ...
    case "nsfw_avatar":
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/random2/${command}?apikey=${apikey}`,
        },
      });
      break;
    ...
    case "ppcouple":
      axios
        .get(`https://api.lolhuman.xyz/api/random/${command}?apikey=${apikey}`)
        .then(({ data }) => {
          sock.sendMessage(from, {
            image: { url: data.result.male },
            caption: "Male",
          });
          sock.sendMessage(from, {
            image: { url: data.result.female },
            caption: "Female",
          });
        });
      break;
    ...
    case "thunder":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${full_args}`,
        },
      });
      break;
    ...
    case "carvedwood":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${full_args}`,
        },
      });
      break;
    ...
    case "pubg":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`,
        },
      });
      break;
    ...
    case "freefire":
      ...
      sock.sendMessage(from, {
        image: {
          url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${full_args}`,
        },
      });
      break;
    case "fotofilter":
    ...
    case "artinama":
      if (args.length === 0) {
        return reply(`Contoh: ${prefix + command} nama`);
      }

      const name = args.join(" ");
      const artiNamaApiUrl = `https://api.lolhuman.xyz/api/artinama?apikey=${apikey}&nama=${name}`;

      axios
        .get(artiNamaApiUrl)
        .then(({ data }) => {
          const arti = data.result;
          reply(`Arti nama *${name}*: ${arti}`);
        })
        .catch((error) => {
          console.error(error);
          reply("Terjadi kesalahan saat mengambil arti nama.");
        });
      break;
    // ...
    // ...
    case "cekip":
      if (args.length === 0) {
        return reply(`Contoh: ${prefix + command} 114.142.169.38`);
      }

      const ipAddress = args[0];
      const ipAddressApiUrl = `https://api.lolhuman.xyz/api/ipaddress/${ipAddress}?apikey=${apikey}`;
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          const message =
            `Informasi IP Address *${ipAddress}*:\n\n` +
            `Negara: ${result.country}\n` +
            `Provinsi: ${result.region}\n` +
            `Kota: ${result.city}\n` +
            `Kode Pos: ${result.zip}\n` +
            `Koordinat: Latitude ${result.lat}, Longitude ${result.lon}\n` +
        ...
        .catch((error) => {
          console.error(error);
          reply("Terjadi kesalahan saat mengambil informasi IP address.");
        });
      break;
    // ...
    // ...
    case "socialsearch":
      if (args.length === 0) {
        return reply(`Contoh: ${prefix + command} John Smith`);
      }

      const queryName = args.join(" ");
      const socialSearchApiUrl =
        "https://social-links-search.p.rapidapi.com/search-social-links";
      const socialSearchHeaders = {
        "X-RapidAPI-Key": "8c69b5ab83msh149e7e2e7698b2fp10c7c1jsn043be9234512",
      ...
      axios
        .get(socialSearchApiUrl, {
          params: socialSearchParams,
          headers: socialSearchHeaders,
        })
        .then(({ data }) => {
          const socialLinksData = data.data;
          let message = `Tautan Sosial untuk "${queryName}":\n\n`;

          for (const network in socialLinksData) {
            message += `${network.toUpperCase()}:\n`;
            for (const link of socialLinksData[network]) {
              message += `${link}\n`;
            }
            message += "\n";
          }
        ...
        .catch((error) => {
          console.error(error);
          reply("Terjadi kesalahan saat mencari tautan sosial.");
        });
      break;
    // ...
    case "hashtag":
      if (args.length === 0) {
        return reply(`Contoh: ${prefix + command} #viralsma`);
      }

      const hashtag = args[0];
      const hashtagSearchApiUrl =
        "https://twitter154.p.rapidapi.com/hashtag/hashtag";
      const hashtagSearchHeaders = {
        "X-RapidAPI-Key": "8c69b5ab83msh149e7e2e7698b2fp10c7c1jsn043be9234512",
      ...
      axios
        .get(hashtagSearchApiUrl, {
          params: hashtagSearchParams,
          headers: hashtagSearchHeaders,
        })
        .then(({ data }) => {
          const tweets = data.data.tweets;
          if (tweets && tweets.length > 0) {
            let message = `Tweets Terkait Hashtag ${hashtag}:\n\n`;
            for (const tweet of tweets) {
              message += `Username: @${tweet.username}\n`;
              message += `Tweet: ${tweet.text}\n`;
              message += `Media URL: ${
                tweet.media_url && tweet.media_url[0]
              }\n`;
              message += `Profile Pic URL: ${tweet.user.profile_pic_url}\n\n`;
            }
            reply(message);
          } else {
            reply(`Tidak ada tweet terkait hashtag ${hashtag}.`);
          }
        })
        .catch((error) => {
          console.error(error);
          reply("Terjadi kesalahan saat mencari hashtag.");
        });
      break;
    ...
    case "cariig":
      ...
      const searchUrl = `https://www.instagram.com/web/search/topsearch/?query=${query}`;
      ...
      try {
        ...
        if (users.length === 0) {
          return reply(
            "Tidak ditemukan pengguna yang sesuai dengan kata kunci tersebut."
          );
        }

        let resultMessage = "Hasil pencarian:\n";
        users.forEach((user, index) => {
          const username = user.user.username || "Username Tidak Tersedia";
          const fullName = user.user.full_name || "Nama Lengkap Tidak Tersedia";
          resultMessage += `${
            index + 1
        ...
        reply("Terjadi kesalahan saat melakukan pencarian di Instagram.");
      }
      break;
    ...
    default:
      if (isCmd) {
        reply(
          `Sorry bro, command *${prefix}${command}* gk ada di list *${prefix}menu*`
        );
      }
      break;
  }
